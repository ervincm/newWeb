...
public class Service {
	...
	public Boolean changeVipPwd(String vipName, String password, String vipType) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			long days = StringAndDate.getDays(dataString, list.get(i)
					.getDeadlineTime());
			if (days > 0) {// 用户会员时间到期
				// 删除vipCountsinfo的信息
	...
	public VipCount loginVip(String username, String userMac, String vipType) {
		...
		if (vipUsersInfo.getVipCountByType(vipType).getIsvip() == null
			...
			if (waitUsersInfoDao.loadUsers(userMac, vipType) != null) {// 分配到的账号因退出被别人占用，且未能成功切换到账号，已处于排队队列中，重新分配账号
				...
				if(StringAndDate.getMinutes(StringAndDate.getStringDate(), loginTime)<10){//前后间隔时间小于10分钟切换账号
					...
					VipCount oldVipCount=vipUsersInfo.getVipCountByType(vipType);
		...
		if(vipType.equalsIgnoreCase("tencent")){
			...
			vipUsersInfo.setTencentLoginTime(StringAndDate.getStringDate());
		}else if (vipType.equalsIgnoreCase("iqiyi")) {
			vipUsersInfo.setIqiyiVipInfo(vipCount);
			vipUsersInfo.setIqiyiLoginTime(StringAndDate.getStringDate());
		}else {
			vipUsersInfo.setYoukuVipInfo(vipCount);
			vipUsersInfo.setYoukuLoginTime(StringAndDate.getStringDate());
		}
		vipUsersInfoDao.updateUsersWithId(vipUsersInfo);
	...
	public VipCount changeVipCount2(String username, String userMac,// 第二次方案下的更换账号
		...
		if (vipUsersInfo.getUseChangeDay() == null) {// 没有记录的时候，首次填入信息
			vipUsersInfo.setUseChangeDay(StringAndDate.getStringDateShort());
			vipUsersInfo.setUseChangeTimes("1");
		} else {
			if (StringAndDate.getDays(StringAndDate.getStringDateShort(),
					vipUsersInfo.getUseChangeDay()) > 0) {// 有记录，但是已经过来一天
				vipUsersInfo
						.setUseChangeDay(StringAndDate.getStringDateShort());
				vipUsersInfo.setUseChangeTimes("1");

			} else if (StringAndDate.getDays(
					StringAndDate.getStringDateShort(),
					vipUsersInfo.getUseChangeDay()) == 0) { // 有记录，但是同一天
	...
	public VipCount changeVipCount(String username, String userMac,// 第一次的方案的切换账号
		...
		if (vipUsersInfo.getUseChangeDay() == null) {// 没有记录的时候，首次填入信息
			...
			if (StringAndDate.getDays(StringAndDate.getStringDateShort(),
					vipUsersInfo.getUseChangeDay()) > 0) {
				vipUsersInfo
						.setUseChangeDay(StringAndDate.getStringDateShort());
				vipUsersInfo.setUseChangeTimes("1");
			}
	...
	public boolean getOldVipCount(VipCount vipCount,String vipType,VipUsersInfo vipUsersInfo) {
		VipCountsInfoDao vipCountsInfoDao=new VipCountsInfoDao();
		VipCountsInfo vipCountsInfo=vipCountsInfoDao.loadVipCount(vipCount.getName(), vipType);
		if(vipCountsInfo==null){
			return false;
		}
		if(Integer.parseInt(vipCountsInfo.getAssignedNum())>=5){//账号被分完
			...
			updateAddVipCountsInfo(vipCountsInfo, vipUsersInfo.getCity(), vipUser2);
		}
		return true;
	}
	
	public void waitVipCount(VipUsersInfo vipUsersInfo,String vipType,String username,String userMac) {
		VipCountsInfoDao vipCountsInfoDao=new VipCountsInfoDao();
		WaitUsersInfoDao waitUsersInfoDao=new WaitUsersInfoDao();
		// 同城市下，同类型vip账号可分配人数<等待人数，则不再排队
		List<?> list = vipCountsInfoDao.listCountsByCityAndViptype(
